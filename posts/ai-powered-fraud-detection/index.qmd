...
This project bridges that gap! - By combining Power BI with an Isolation Forest anomaly detection model, Groq's Llama API, and Power BI's native analytics like Key Influencers and smart narratives, this dashboard moves beyond basic reporting into automated, AI-powered explanation.
...
The pipeline runs in five stages: synthetic claims data is generated in Python, anomalies are detected using an Isolation Forest model, risk scores are assigned based on the anomaly score distribution, and the results are loaded into Power BI alongside the raw features. These results are then explained in two ways—first through Power BI's native Key Influencers visual, and second through a Groq-hosted Llama model that transforms the numeric evidence into an investigator-readable narrative.
